...
var decode = function(txt) {
  ...
  return {
    locn: metar.station,
    txt: txt,
    time : moment(metar.time).add(4, 'hours').format('DD/MM HH:mm'),
...
var getMETAR = function(code, numReadings, cb){
  var url = 'http://aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=xml&stationString='+ code + '&hoursBeforeNow=' + 20;
  request(url, function (error, response, data) {
    ...
    parser(data, function (err, jsonRes) {
      ...
      var parsedObj = _.map(json , function(o){
        return decode(o.raw_text[0]);
      } )

      // _.each(parsedObj, function(o){
